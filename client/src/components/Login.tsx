...
const Login = () => {
    ...
    const fillDemoCredentials = () => {
        setState("login"); // Switch to login tab if on signup
        setFormData({
            name: "Demo User", 
        ...
        <>
            ...
            <div className='min-h-screen flex items-center justify-center'>
                ...
                >
                    ...
                    {/* REPLACED FORGOT PASSWORD WITH DEMO LOGIN BUTTON */}
                    <div className="mt-4 text-left flex items-center gap-2">
                        <span className="text-sm text-gray-500">Testing?</span>
                        <button 
                            type="button" 
                            onClick={fillDemoCredentials}
                            className="text-sm text-pink-400 hover:underline hover:text-pink-300 transition-colors"
                        >
                            Click to fill Demo Login
                        </button>
                    </div>

                    <button type="submit" className="mt-4 w-full h-11 rounded-full text-white bg-pink-600 hover:bg-pink-500 transition shadow-lg shadow-pink-500/20">
                        {state === "login" ? "Login" : "Sign up"}
                    </button>

                    <p onClick={() => setState(prev => prev === "login" ? "register" : "login")} className="text-gray-400 text-sm mt-3 mb-11 cursor-pointer select-none">
